...
for split in SPLITS:
    ...
    mkdir(out_path)
    if 'half' in split:
        gt_name = f'gt_{split}.txt'
    else:
        gt_name = 'gt.txt'

    for seq in tqdm(os.listdir(os.path.join(DATA_PATH,'train'))):
        if 'FRCNN' not in seq:
            continue
        # read gt of all players
        with open(os.path.join(DATA_PATH,'train',seq,'gt',gt_name),'r') as f:
            gt=f.readlines()
        ...
        for line in gt:
            ...
            if int(line[6]) != 1 or int (line[7]) != 1:
                # ignore
                continue
        ...
        length = len(os.listdir(os.path.join(DATA_PATH,'train',seq,'img1')))
        if split == 'train_half':
            length = length//2+1
        elif split == 'val_half':
